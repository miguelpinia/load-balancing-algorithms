 ...
 * @author miguel
 */
public class FIFOWorkStealingV1SpanningTree implements SpanningTree {
    ...
    public void spanningTree(int shape, int numThreads, boolean displayInfo, boolean randomRoots, GraphType type) {
        ...
        for (int i = 0; i < numThreads; i++) {
            threads[i] = new Thread(new StepFIFOWorkStealingV1(graph,
        ...
        for (int i = 0; i < numThreads; i++) {
            try {
                ...
                Logger.getLogger(StepFIFOWorkStealingV1.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
