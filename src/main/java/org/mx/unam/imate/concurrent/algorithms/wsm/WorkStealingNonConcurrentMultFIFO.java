 ...
 * @author miguel
 */
public class WorkStealingNonConcurrentMultFIFO implements WorkStealingStruct {
    ...
    public int take(int label) {
        ...
        if (head[label] <= tail[label]) {
            ...
            if (p.getTask() >= 0 && p.getState() == State.TRUE) {
                Tasks.set(head[label], new Pair(p.getTask(), State.FALSE));
