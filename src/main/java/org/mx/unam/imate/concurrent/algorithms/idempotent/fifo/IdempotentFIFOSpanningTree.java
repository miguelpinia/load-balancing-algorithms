 ...
 * @author miguel
 */
public class IdempotentFIFOSpanningTree implements SpanningTree {
    ...
    public Graph spanningTree(int shape, int numThreads, boolean displayInfo, boolean randomRoots, GraphType type) {
        ...
        IdempotentWorkStealingFIFO[] queues = new IdempotentWorkStealingFIFO[numThreads];
        for (int i = 0; i < numThreads; i++) {
            queues[i] = new IdempotentWorkStealingFIFO(128);
        }
        if (displayInfo) {
            System.out.print("[");
        ...
        for (int i = 0; i < numThreads; i++) {
            threads[i] = new Thread(new StepIdempotentFIFOSpanningTree(graph,
